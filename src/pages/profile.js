...
function ProfileNameSection({ user, isOwner, handleOptionsMenuClick }) {
  ...
  if (isFollowing) {
    ...
    followButton = (
      <Button onClick={handleFollowUser} variant="contained" color="secondary" className={classes.button}>
        Follow Back
      </Button>
    );
  } else {
...
function UnfollowDialog({ onClose, user, onUnfollowUser }) {
  ...
  return (
    ...
    >
      ...
      <Divider />
      <Button onClick={handleUnfollowUser} className={classes.unfollowButton}>Unfollow</Button>
      <Divider />
      ...
      <section className={classes.followingSection}>
        {options.map((option) => (
          ...
          <div key={option} className={classes.followingText}>
            ...
            <Hidden xsDown>
              <Typography>{option === 'favorite_movies' ? 'favorites' : option}</Typography>
            </Hidden>
            <Hidden smUp>
              <Typography color="textSecondary">{option === 'favorite_movies' ? 'favorites' : option}</Typography>
            </Hidden>
          </div>
        ))}
      </section>
